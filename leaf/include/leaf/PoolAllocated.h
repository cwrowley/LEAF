/*
 ...
 2. Define a single constructor:  Foo(LeafContext ctx, <extra args>...)
...
struct LeafContext {
    ...
    LeafContext(LEAF &l)             : leaf(l), mempool(*l.mempool()) {}
    /// Explicit construction with a specific mempool.
    LeafContext(LEAF &l, Mempool &m) : leaf(l), mempool(m) {}
};
...
public:
    ...
    static Derived *create(LEAF &leaf, Args &&...args) {
        ...
        return new (mem) Derived(LeafContext{leaf, m}, std::forward<Args>(args)...);
    }
...
protected:
    explicit PoolAllocated(LeafContext ctx) : mempool_(&ctx.mempool) {}
    Mempool *mempool_;
