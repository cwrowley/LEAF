...
Lfloat tTalkboxLfloat_tick(tTalkboxLfloat *const v, Lfloat synth, Lfloat voice) {
    ...
    if (v->K++) {
        ...
        v->buf0[p0] = x * w; // 50% overlapping hanning windows
        if (++p0 >= v->N) {
            tTalkboxLfloat_lpc(v->buf0, v->car0, v->dl, v->Rt, v->N, v->O, v->warpFactor, v->warpOn, v->k, v->freeze, &v->G);
            p0 = 0;
        }
        ...
        v->buf1[p1] = x * w;
        if (++p1 >= v->N) {
            tTalkboxLfloat_lpc(v->buf1, v->car1, v->dl, v->Rt, v->N, v->O, v->warpFactor, v->warpOn, v->k, v->freeze, &v->G);
            p1 = 0;
        }
    }
...
#endif
    if (fabs(o) > 10.0f)
        tVocoder_suspend(v); // catch instability
