...
void tFeedbackLeveler_setStrength(tFeedbackLeveler *const p, Lfloat strength) { // strength is how strongly level diff is affecting the amp ratio
...
void tFeedbackLeveler_setMode(tFeedbackLeveler *const p, int mode) { // 0 for decaying with upwards lev limiting, 1 for constrained absolute level (also downwards limiting)
    p->mode = mode;
}
