...
void tVoc_set_tongue_and_touch_diameters(tVoc *const v, Lfloat tongue_index, Lfloat tongue_diameter, Lfloat touch_index, Lfloat touch_diameter, Lfloat *theDiameters) {
    ...
    */
    ...
    if (touch_index < (twentyfivewidth))
        width = tenwidth; // 25 in original code
    else if (touch_index >= v->tr->tip_start)
        width = fivewidth;
    else
        width = tenwidth - fivewidth * (touch_index - twentyfivewidth) / (v->tr->tip_start - twentyfivewidth);
    ...
    if ((touch_index < v->tr->n) && (touch_diameter < 3.0f)) {
        ...
        for (int i = -ceilf(width) - 1.0f; i < width + 1.0f; i++) {
            if (intIndex + i < 0 || intIndex + i >= v->tr->n)
                continue;
            ...
            if (relpos <= 0.0f)
                ...
                shrink = 0.5f * (1.0f - arm_cos_f32(M_PI * relpos * invWidth));
#else
            else
                shrink = 0.5f * (1.0f - cosf(M_PI * relpos * invWidth));
#endif
